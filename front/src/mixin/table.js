...
export default {
  methods: {
    criardatatable(rota, colunas, parametro, avancada, serverside = false, token, rota401redirect = '/logout', colunastamanho = [], ordercampo = 0, ordertype = 'asc') {
      let Myapp = this;
      /*!
      ...
      */
      ...
      //inicio datatable sistema
      $('#users-table').DataTable().destroy();
      if (parametro) {
        var oTable = null;
        oTable = $('#users-table').DataTable({
          order: [
            [ordercampo, ordertype]
          ],
          deferLoading: 0,
          pageLength: 5,
          autoWidth: true,
          ...
          fnDrawCallback: function (oSettings, json) {
            $("#users-table_paginate .page-link").addClass("q-btn bg-primary text-white");
            $("#users-table_paginate .page-link").css("margin", "-18px");
            $("#users-table_paginate .active .page-link").removeClass("q-btn bg-primary text-white");
            $("#users-table_paginate .active .page-link").addClass("q-btn bg-blue-10 text-white text-center justify-center q-btn__wrapper");
          ...
          initComplete: function () {
            this.api().columns().every(function () {
              ...
              $('#users-table_filter input').attr('placeholder', 'Pesquisar...');
              //   $('#users-table_filter input').addClass('q-field__native');
              ...
              $('#users-table tfoot tr').css('border', 'none');
              $('#users-table tfoot tr').appendTo('#users-table thead');
              $('#users-table thead tr th input').addClass('form-control');
              $('#users-table thead tr th input').attr('style', 'width: 100%');
              $('#users-table thead tr th input').attr('placeholder', 'Buscar...');
              $("#users-table_paginate .page-link").addClass(
                "q-btn bg-primary text-white  text-center justify-center q-btn__wrapper"
              );
              $("#users-table_paginate .active .page-link").removeClass(
                "q-btn bg-primary text-white"
              );
              $("#users-table_paginate .active .page-link").addClass(
                "q-btn bg-blue-10 text-white text-center justify-center q-btn__wrapper"
              );
              $("#users-table_paginate .page-link").css("margin", "-18px");
              //   if ($(window).width() <= 425) {
              $('.buttons-excel').hide();
          ...
          ajax: {
            ...
            error: function (reason) {
              ...
              if (reason.status == 403) {
                alert(reason.responseJSON['title'] + ' ' + reason.responseJSON['message']);
                Myapp.$router.go(-1);
              }
              if (reason.status == 401) {
                alert(reason.responseJSON['title'] + ' ' + reason.responseJSON['message']);
                Myapp.$router.push(rota401redirect);
              }
            }
          },
        ...
        // $('#users-table input').attr('placeholder', 'Buscar...');
